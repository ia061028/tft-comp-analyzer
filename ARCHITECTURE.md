...
- 「採用1%以上」で、表示中（パッチ・レベル）の参加者の1%に満たない行を外せる（既定は全部出す）。固定人数だと母数が大きいパッチでは何も外れないため割合にしている。
- **レベルと選択駒で絞れる**（レベルは 全Lv / 〜7 / 8 / 9 / 10。7以下と10以上はまとめる）。summary.json の各ビューは `cells` に「レベル区分 × 選択駒の有無（ビット）」ごとの同じ集計を持ち、画面が条件に合う区分を足す（`filterView`）。1人は1区分だけに入るので、足せば正確な値になる。掘り下げはレベル区分ごとに `drill-<ビュー>-lv<区分>.json` を別に書く（区分を選んで行を開いたときに読む）。選択駒で絞っている間は、構成の型は開けない（組み合わせごとにファイルを作ると数十個になるため）。

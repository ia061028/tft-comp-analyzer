...
紋章の選択に**重ねて**、盤面ユニットで構成を絞る（`src/lib/unitFilter.ts`）。紋章と同じレール／ドックを「紋章｜チャンピオン」で切り替えて使い、タイルを押すたびに 印なし → 使う → 使わない → 印なし と回る。使うに選んだ駒が全員居て、使わないに選んだ駒が誰も居ない構成だけが残る。
...
- 紋章を選ばずチャンピオンだけで一覧を出すことはしない（2026-09-23 決定）。行は紋章活用シグネチャから作っていて、紋章を活用していない試合（全体の約3分の2）の成績は出力に無いため。

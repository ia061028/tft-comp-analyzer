...
- 付与元ユニットはファイル単位の `granters = [unitIdx, traitIdx, delta]` で持つ。「そのトレイトが `delta` だけ上乗せされたレコードのうち、そのユニットが盤面に居た割合」が `GRANTER_MIN_COVERAGE` 以上のユニットを1体だけ採る。誰も閾値に届かなければ**発動数は正しいまま「誰の分か」の表示だけが落ちる**。
  - **上乗せ数ごとに分けて数える**。同じトレイトでも由来ごとに上乗せ数が違い、混ぜると本来の付与元の同席率が薄まる。実測でフェイの +2 はラックス同席率 100% だが、+1（ラックス同席率 12.7%・オーグメント由来と見られる）を混ぜると 80.7% まで落ちて閾値を割っていた。
  ...
  - それでも残る未確定がある。魔女とソーラーの +2 はラックス同席率が 68.2% / 86.4% で、ラックス以外の経路（オーグメントと見られる）が混じる。**収集レコードはオーグメントを持たない**ので、この2つは分離できない。分離するには `participant.augments` の収集から必要になる。

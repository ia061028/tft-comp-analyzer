...
- 付与元ユニットはファイル単位の `granters = [unitIdx, traitIdx]` で持つ。「そのトレイトの上乗せが観測されたレコードのうち、そのユニットが盤面に居た割合」が `GRANTER_MIN_COVERAGE` 以上のユニットを1体だけ採る。1トレイトを複数ユニットが付与するセットでは誰も閾値に届かず、**発動数は正しいまま「誰の分か」の表示だけが落ちる**。

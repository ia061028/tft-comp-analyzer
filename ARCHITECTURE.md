...
- 付与元ユニットはファイル単位の `granters = [unitIdx, traitIdx, delta]` で持つ。「そのトレイトが `delta` だけ上乗せされたレコードのうち、そのユニットが盤面に居た割合」が `GRANTER_MIN_COVERAGE` 以上のユニットを1体だけ採る。誰も閾値に届かなければ**発動数は正しいまま「誰の分か」の表示だけが落ちる**。
  ...
  - それでも残る未確定がある。魔女とソーラーの +2 はラックス同席率が 68.2% / 86.4% で、ラックス以外の経路が混じる。**ユニット由来でない上乗せが常に一定量ある**ためで、実測では特殊チャンピオン3体がどれも居ない 67,467 レコードの 2.9% に上乗せがあり、30 種類の (トレイト, 上乗せ数) に散っていた。魔女 +2 のラックス不在 148 件では最も同席率の高いユニットでも 56% で、特定のユニットには寄らない（セット18 のウィスプのような一時効果と見られる）。付与元がユニットでない以上この2つは確定できないので、発動数だけ正しく入りバッジは出ない。

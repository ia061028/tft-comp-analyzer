...
紋章ごと・特性の発動段ごとの平均順位・Top4率・1位率・平均レベル・採用率を、別ページ（`/stats.html`）に表で出す。構成一覧（`/`）とはヘッダーの「構成／統計」で行き来する。
...
- **並び順は縮約した平均順位**（`shrunk`、仮想10試合ぶん 4.5 に寄せる）。**採用が少ないこと自体では下げない**（難しいが実現すれば強い段は上に来るべき）ので、頻度は別の列（採用率）で見せる。人数が十分な行は生の値とほぼ同じ順になる。

...
export const config = {
  ...
  /**
   * ローリング窓: records/{route}.ndjson に保持する最大バイト数。
   *
   * GitHub のハード上限は 100MB/ファイルで、超えると push が弾かれて収集が完全停止する。
   * 件数で切るとレコードの実サイズ次第で上限に当たるため、バイト数で直接切る。
   * 88MB は上限に対して12%の余裕。セット18 の実測（約705バイト/レコード）だと
   * 1ルート約125,000レコード＝約15,600マッチ、4ルートで約62,000マッチが定常サンプル。
   * レコードが大きくなればマッチ数が自動的に減るだけで、上限には当たらない。
   */
  maxBytesPerRoute: 88 * 1024 * 1024,

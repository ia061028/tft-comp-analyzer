...
export const config = {
   ...
   * GitHub のハード上限は 100MB/ファイル。セット18 の実データで計測したところ
   * 約705バイト/レコード（4ルートで 703〜723）だったので、余裕を見て 720バイト換算とし
   * 120,000 レコード ≒ 86MB（約15,000マッチ）を上限にする。4ルートで約60,000マッチが定常サンプル。
   * 当初は 900バイト/レコードと見積もって 90,000 にしていたが、それだと 63MB にしかならず
   * 上限まで 37MB 遊んでいた。
   */
  maxRecordsPerRoute: 120000,

...
export const config = {
  ...
  /**
   * 直近 N 日ビュー（鮮度重視）。既定パッチの中だけで、最新の試合から N 日さかのぼった分を集計する。
   * パッチ全体と同じ中身になる窓（パッチ配信から N 日未満）は出さない。
   */
  recentWindowDays: [3, 1],
  /**
   * 最初に開くビュー。直近 N 日ビューが出ていればそれ、出ていなければ既定パッチ。null で常に既定パッチ。
   */
  defaultRecentDays: 3 as number | null,
  /**
   * 1ビュー（stats-*.json）に出力する構成数の上限（0 で無制限）。
   * 「紋章活用レコード数の上位」と「総レコード数の上位」をそれぞれこの数で切った和集合を出すので、
   * 実際の構成数はこれより多い（18.2b の実測で 20,000 → 約25,000、約7MB）。
   * データ量が増えるほど n>=3 の構成が増えてファイルが肥大化するため、ここで抑える。
   */
   ...
   *
   * since は UTC。通常パッチの配信はリージョンごとに数時間ずれる（OCE→KR→EU→NA の順で
   * 前日夜〜当日昼 UTC）が、配信前後はメンテナンスで試合がほぼ無いので配信日の 00:00Z で足りる。
   * ミッドパッチ（B パッチ。例 "18.2b"）はメンテナンス無しで配信されるため境界が曖昧にならず、
   * 00:00Z ではなく**実際の配信時刻**を入れる（ずれた分だけ前後のレコードが誤ったパッチに入る）。
   * 新パッチ・B パッチごとに1行追加する。未登録の期間のレコードは

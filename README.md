...
- **フィルタ帯は2段**: 上の段が「何を見て、どう並べるか」（パッチ・盤面サイズ・並び替え・まとめ）、下の段が「何を外すか」（4つの絞り込みと解除）。特性ラダーと生涯ブロンズは同時に立たないので、2つのトグルではなく「まとめ」の1つの選択にまとめ、選択中の色を行に出る数字の色（ラダー＝金 / ブロンズ＝銅）に合わせている。モバイルの畳んだ帯には効いている絞り込みの**数**だけを出す（値を並べると帯が2段になり、畳んでいる意味が無くなる）。

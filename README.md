...
- **採用数は隠さず見せる**: 採用数（その盤面をその紋章の使い方で組んだ試合数）を各行に数字＋4段階のメーターで出す。**採用数による足切りはしない** — 下限で切ると紋章を2枚以上使う構成がほぼ全部消えるため（18.2b の実データで2枚使う行 14,121 件のうち 76% が採用数1、かつての既定の下限5を超えるのは 4.8% だけ）。少サンプルの極端な率が上位に来る問題は並び順側の縮約（`shrunk`）が抑える。目障りなときは「少数を薄く」トグルで採用数2以下を淡くできる（消さない）。長い一覧は50件ずつ表示する。

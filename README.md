...
- **採用数は隠さず見せる**: 採用数（その盤面をその紋章の使い方で組んだ試合数）を各行に数字＋4段階のメーターで出す。**「採用数」の下限は既定 1 ＝ 何も外れない。** かつて既定 5 だった頃は紋章を2枚以上使う構成がほぼ全部消えていた（18.2b の実データで2枚使う行 14,121 件のうち 76% が採用数1、下限5を超えるのは 4.8% だけ）ので、既定で隠すのはやめ、絞りたいときだけ自分で上げる入力にした。少サンプルの極端な率が上位に来る問題は並び順側の縮約（`shrunk`）が抑える。「少数を薄く」トグルは消さずに淡くするだけの別の手段。長い一覧は50件ずつ表示する。
